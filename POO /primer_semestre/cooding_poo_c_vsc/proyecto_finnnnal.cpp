...
// filtrado por apellido 
    void listarPorApellido(const std::string& apellido) {
    limpiarPantalla();
    std::ifstream arch("alumnos.csv");
    if (!arch) {
        std::cout << "Error al abrir el archivo.\n";
    ...
    while (std::getline(arch, line)) {
        std::istringstream iss(line);
        std::string matricula, nombre, appaterno, apmaterno, calif;
        std::getline(iss, matricula, ',');
        std::getline(iss, nombre, ',');
        std::getline(iss, appaterno, ',');
        std::getline(iss, apmaterno, ',');
        std::getline(iss, calif, ',');

        // Filtra por el apellido paterno
        if (appaterno == apellido) {
            std::cout << std::setw(10) << matricula 
                      << std::setw(15) << nombre 
                      << std::setw(20) << appaterno 
                      << std::setw(20) << apmaterno 
                      << std::setw(5) << calif << "\n";
        }
    }
    arch.close();
...
void menu() {
    ...
    do {
        ...
        std::cout << "4) Listar alumnos por apellido\n";
        std::cout << "5) Terminar\n";
        std::cout << "Indique la opción deseada: ";
        std::cin >> op;
        std::cin.ignore(); // Ignora el '\n' que queda en el buffer después de std::cin
        ...
        switch (op) {
            ...
            case 4: {
                ...
                Archivo obj;
                obj.listarPorApellido(apellido);
                break;
        }
